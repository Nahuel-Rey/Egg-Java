...
WHERE NOT EXISTS (SELECT D.NOMBRE_DEPTO FROM DEPARTAMENTOS WHERE D.NOMBRE_DEPTO='VENTAS'OR D.NOMBRE_DEPTO='INVESTIGACION' OR D.NOMBRE_DEPTO='MANTENIMIENTO');

SELECT NOMBRE,SAL_EMP FROM EMPLEADOS WHERE (SELECT MAX(SAL_EMP) FROM EMPLEADOS) ORDER BY SAL_EMP DESC LIMIT 1;
